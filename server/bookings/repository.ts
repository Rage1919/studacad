...
import { meetLinkReleaseMinutes, meetingDetailsView } from "../meet/policy.mjs";
...
const databaseMessage = (
  ...
  const safe = [
    ...
    "The selected group is full",
    "Learner is already booked into this slot",
    "Bookings cannot be cancelled after the lesson starts",
    "You cannot cancel this booking",
    "Booking not found",
    "Only an active approved tutor may manage availability",
    "Every rule must use an approved session format",
    "Only the booked tutor may record this outcome",
    "The lesson outcome cannot be recorded yet",
    "Only an active booking participant may raise a dispute",
    "This booking is outside the dispute window",
    "Unsupported booking outcome",
    "Learner already has an overlapping active booking",
    "Idempotency key was already used for a different booking",
  ].find((candidate) => message.includes(candidate));
  if (safe)
    return new BookingError(
      safe,
      safe === "Booking not found"
        ? 404
        : safe.includes("cannot") || safe.includes("Only")
          ? 403
          : 409,
    );
...
export async function listAvailableSlots(input: {
  ...
  const { data, error } = await getDatabaseAdminClient().rpc(
    "list_tutor_slots",
    {
...
export async function getOwnAvailability(userId: string) {
  ...
  const profile = await database
    ...
    .maybeSingle();
  if (profile.error)
    throw databaseMessage(profile.error, "Unable to load the tutor profile.");
  if (!profile.data)
    throw new BookingError(
      "An approved active tutor profile is required.",
      403,
    );
  ...
  if (rules.error || exceptions.error)
    throw databaseMessage(
      rules.error ?? exceptions.error,
  ...
  const [subjects, formats] = await Promise.all([
    database
      ...
      .order("examination"),
    database
      .from("tutor_profile_formats")
      .select("*")
      .eq("tutor_profile_id", profile.data.id)
      .order("format"),
  ]);
  if (subjects.error || formats.error)
    throw databaseMessage(
      subjects.error ?? formats.error,
      "Unable to load tutor settings.",
    );
  return {
    profile: {
      id: profile.data.id,
    ...
    rules: rules.data.map((rule) => ({
      ...
      localEndTime: rule.local_end_time.slice(0, 5),
      timezone: rule.timezone,
      format: rule.format,
      slotDurationMinutes: rule.slot_duration_minutes,
      leadTimeMinutes: rule.lead_time_minutes,
    ...
    settings: {
      subjects: subjects.data.map((subject) => ({
        ...
        priceCredits: subject.price_credits,
      })),
      formats: formats.data.map((format) => ({
        format: format.format,
        groupCapacity: format.group_capacity,
        locationNote: format.location_note ?? "",
      })),
    },
  };
}
...
export async function recordBookingOutcome(input: {
  actorUserId: string;
  bookingId: string;
  targetStatus: "completed" | "no_show" | "disputed";
  reason: string;
  idempotencyKey: string;
}) {
  const { data, error } = await getDatabaseAdminClient().rpc(
  ...
  if (error)
    throw databaseMessage(error, "Unable to update the booking outcome.");
  return data;
}

export async function listBookingsForViewer(viewer: Viewer) {
  const database = getDatabaseAdminClient();
  const ownParticipants = await database
    .from("booking_participants")
    .select("*")
    .eq("learner_user_id", viewer.id);
  if (ownParticipants.error)
    throw databaseMessage(ownParticipants.error, "Unable to load bookings.");
  const ownParticipantMap = new Map(
    ownParticipants.data.map((participant) => [
      participant.booking_id,
      participant,
    ]),
  );
  const bookingIds = new Set(
    ownParticipants.data.map((participant) => participant.booking_id),
  );
  ...
  if (viewer.roles.includes("tutor")) {
    const profile = await database
      ...
      .maybeSingle();
    if (profile.error)
      throw databaseMessage(profile.error, "Unable to load tutor bookings.");
    tutorProfileId = profile.data?.id ?? null;
    if (tutorProfileId) {
      const tutorBookings = await database
        .from("bookings")
        .select("*")
        .eq("tutor_profile_id", tutorProfileId);
      if (tutorBookings.error)
        throw databaseMessage(
          tutorBookings.error,
          "Unable to load tutor bookings.",
        );
  ...
  if (bookings.error)
    throw databaseMessage(bookings.error, "Unable to load bookings.");
  const profileIds = [
    ...new Set(bookings.data.map((booking) => booking.tutor_profile_id)),
  ];
  const profiles = await database
    ...
    .in("id", profileIds);
  if (profiles.error)
    throw databaseMessage(profiles.error, "Unable to load booking tutors.");
  const tutorUserIds = [
    ...new Set(profiles.data.map((profile) => profile.tutor_user_id)),
  ];
  const accounts = await database
    ...
    .in("id", tutorUserIds);
  if (accounts.error)
    throw databaseMessage(accounts.error, "Unable to load booking tutors.");
  ...
  if (allParticipants.error)
    throw databaseMessage(
      allParticipants.error,
      "Unable to load booking participants.",
    );
  const locations = await database
    .from("booking_location_details")
    .select("*")
    .in("booking_id", [...bookingIds]);
  if (locations.error)
    throw databaseMessage(locations.error, "Unable to load booking locations.");
  const locationsByBooking = new Map(
    locations.data.map((location) => [location.booking_id, location]),
  );
  const meetings = await database
    ...
    .in("booking_id", [...bookingIds]);
  if (meetings.error)
    throw databaseMessage(meetings.error, "Unable to load booking meetings.");
  const meetingsByBooking = new Map(
    meetings.data.map((meeting) => [meeting.booking_id, meeting]),
  );
  ...
  return bookings.data.map((booking) => {
    ...
    const viewerStatus = participant?.cancelled_at
      ? "cancelled_by_learner"
      : booking.status;
    const activeParticipants = allParticipants.data.filter(
      (item) => item.booking_id === booking.id && !item.cancelled_at,
    ).length;
    ...
    if (isTutor && booking.status === "confirmed" && now >= endsAt)
      availableActions.push("completed", "no_show");
    if (
      !isTutor &&
      participant &&
      ...
      now >= startsAt &&
      now <= endsAt + 7 * 86_400_000
    )
      availableActions.push("disputed");
    return {
      id: booking.id,
      tutorName: tutor?.display_name ?? "Studacad tutor",
      ...
      priceCredits: booking.price_per_learner_credits,
      capacity: booking.capacity,
      activeParticipants,
      locationNote:
        location?.learner_address ?? location?.tutor_location_note ?? null,
      status: viewerStatus,
      perspective: isTutor ? "tutor" : "learner",
      canCancel:
        startsAt > now &&
        ["pending", "held", "confirmed"].includes(booking.status) &&
        (isTutor || Boolean(participant && !participant.cancelled_at)),
      availableActions,
      meeting: meetingDetailsView({
        format: booking.format,
        bookingStatus: booking.status,
        startsAt: booking.starts_at,
        meeting: meetingsByBooking.get(booking.id) ?? null,
