...
const nextMatchingSlot = (slots: string[], filter: string) => {
  if (filter === "Today")
    return slots.find((slot) => slot.startsWith("Today")) ?? slots[0];
  if (filter === "Tomorrow")
    return slots.find((slot) => slot.startsWith("Tomorrow")) ?? slots[0];
  if (filter === "Weekend")
    return slots.find((slot) => /^(Sat|Sun)/.test(slot)) ?? slots[0];
  if (filter === "Weekdays")
    return (
      slots.find((slot) => /^(Mon|Tue|Wed|Thu|Fri)/.test(slot)) ?? slots[0]
    );
  return slots[0];
};
...
export default function TutorsPage() {
  ...
  const [marketplaceTutors, setMarketplaceTutors] = useState<Tutor[]>([]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/tutors", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(result.error ?? "Unable to load published tutors.");
      ...
      .catch((error) =>
        setMarketplaceError(
          error instanceof Error
            ? error.message
            : "Unable to load published tutors.",
        ),
      )
  ...
  useEffect(() => {
    ...
    window.history.replaceState(
      null,
      "",
      `/tutors${params.size ? `?${params.toString()}` : ""}`,
    );
  ...
  const subjectOptions = useMemo(
    () =>
      Array.from(
        new Set(
          marketplaceTutors
            .filter((tutor) => exam === "All" || tutor.examination === exam)
            .map((tutor) => tutor.subject),
        ),
      ).sort(),
    [exam, marketplaceTutors],
  );
  ...
  const filteredTutors = useMemo(() => {
    const result = marketplaceTutors.filter((tutor) => {
      ...
      if (
        ...
        !tutor.availabilityGroups.includes(
          availability as "Today" | "Tomorrow" | "Weekdays" | "Weekend",
        )
      )
        return false;
      if (maxPrice !== "Any price" && tutor.price > Number(maxPrice))
        return false;
      if (
        minimumRating !== "Any rating" &&
        Number(tutor.rating) < Number(minimumRating)
      )
        return false;
      if (
        search &&
        !`${tutor.name} ${tutor.subject} ${tutor.examination}`
          .toLowerCase()
    ...
    return [...result].sort((a, b) =>
      sort === "price-low"
        ...
        : sort === "rating"
          ...
          : Number(b.rating) - Number(a.rating) ||
            b.lessons.localeCompare(a.lessons),
    );
  }, [
    availability,
    exam,
  ...
  const changeExam = (value: string) => {
    ...
    if (
      value !== "All" &&
      !marketplaceTutors.some(
        (tutor) => tutor.examination === value && tutor.subject === subject,
      )
    )
      setSubject("All");
  };
  ...
  const title =
    subject !== "All"
      ? `${exam !== "All" ? `${exam} ` : ""}${subject} tutors`
      : exam !== "All"
        ? `${exam} tutors`
        : "Find your subject tutor";

  return (
    <main className="lms-page tutor-search-page">
      <LmsHeader />
      <section className="tutor-search-hero">
        <div>
          <p className="eyebrow">Studacad tutor marketplace</p>
          <h1>{title}</h1>
          <p>
            Compare subject expertise, ratings, prices, and real
            availability—then open a profile and book with credits.
          </p>
        </div>
        <div className="search-count">
          <strong>{filteredTutors.length}</strong>
          <span>matching tutor{filteredTutors.length === 1 ? "" : "s"}</span>
        </div>
      </section>
      ...
      <section className="tutor-results-section">
        <div className="results-toolbar">
          <div>
            <p className="eyebrow">Available tutors</p>
            <h2>{title}</h2>
          </div>
          <div>
            <label>
          ...
          {filteredTutors.map((tutor) => (
            <article className="search-tutor-card" key={tutor.id}>
              ...
              <div className="search-tutor-body">
                <div className="search-tutor-name">
                  <div>
                    <h3>
                      {tutor.name} <i>✓</i>
                    </h3>
                ...
                <div className="search-specialties">
                  {tutor.specialties.slice(0, 3).map((item) => (
        ...
        {marketplaceLoading && (
          <div className="no-tutor-results">
            <h2>Loading verified tutors…</h2>
          </div>
        )}
        {marketplaceError && (
          <div className="no-tutor-results">
            <span>!</span>
            <h2>Tutor marketplace is temporarily unavailable</h2>
        ...
        {ready &&
          ...
          filteredTutors.length === 0 && (
            <div className="no-tutor-results">
              <span>⌕</span>
              <h2>No approved tutors match every filter</h2>
              <p>Try another availability, price, or subject.</p>
