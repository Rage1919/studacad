...
export function LmsProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshLearning = useCallback(async () => {
    try {
      ...
      if (!response.ok)
        throw new Error(payload.error ?? "Unable to load learning data.");
      setState((current) => ({
        ...payload,
        credits: current.credits,
        transactions: current.transactions,
        referralRewards: current.referralRewards,
      }));
      setLearningError("");
    } catch (error) {
      setLearningError(
  ...
  const refreshWallet = useCallback(async () => {
    try {
      ...
      const wallet = (await response.json()) as Pick<
        LmsState,
        "credits" | "transactions"
      > & { balance?: number; error?: string };
      if (!response.ok)
        throw new Error(wallet.error ?? "Unable to load the wallet.");
      setState((current) => ({
        ...current,
        credits: wallet.balance ?? wallet.credits ?? 0,
        transactions: wallet.transactions ?? [],
      }));
      setWalletError("");
    } catch (error) {
      setWalletError(
        error instanceof Error ? error.message : "Unable to load the wallet.",
      );
    } finally {
      setWalletReady(true);
    }
  ...
  const refreshReferrals = useCallback(async () => {
    try {
      ...
      const payload = (await response.json()) as {
        code?: string;
        rewards?: LmsState["referralRewards"];
        error?: string;
      };
      if (!response.ok)
        throw new Error(payload.error ?? "Unable to load referral activity.");
      setReferralCode(payload.code ?? "");
      setState((current) => ({
        ...current,
        referralRewards: payload.rewards ?? [],
      }));
    } catch {
  ...
  useEffect(() => {
    ...
    const incoming = document.cookie
      .split("; ")
      .find((item) => item.startsWith("studacad-referral="))
      ?.split("=")[1];
    if (!incoming) return;
    void fetch("/api/referrals", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code: decodeURIComponent(incoming) }),
    }).then((response) => {
      if (response.ok || response.status === 409)
        document.cookie =
          "studacad-referral=; Path=/; Max-Age=0; SameSite=Lax; Secure";
    });
  }, [referralCode]);
    ...
    async (courseId: string): Promise<ActionResult> => {
      try {
        const response = await fetch("/api/lms/purchases", {
          ...
          body: JSON.stringify({
            courseSlug: courseId,
            idempotencyKey: idempotencyKey("course-purchase"),
            acceptPolicies: true,
          }),
        });
        const payload = (await response.json()) as { error?: string };
        if (!response.ok)
          throw new Error(payload.error ?? "Unable to purchase the course.");
    ...
    async (
      ...
      try {
        const response = await fetch("/api/lms/quizzes", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            lessonId: lessonDatabaseId,
            answers,
            idempotencyKey: idempotencyKey("quiz-attempt"),
          }),
        });
        const payload = (await response.json()) as {
        ...
        if (!response.ok || !payload.attempt)
          throw new Error(payload.error ?? "Unable to submit the quiz.");
        await refreshLearning();
        return {
          ok: true,
          message: payload.attempt.passed
            ? "Lesson passed."
            : "Keep practising and try again.",
          score: payload.attempt.scorePercent,
  ...
  const value = useMemo<LmsContextValue>(
    () => ({
      ...
      bookTutor: () => ({
        ok: false,
        message: "Choose a real available slot to book this tutor.",
      }),
