...
export default function LearningHub() {
  ...
  const buy = async (courseId: string) => {
    if (
      !window.confirm(
        "Purchase this course with credits and accept the current Terms and Cancellation and Refund Policy?",
      )
    )
      return;
  ...
  return (
    <main className="lms-page">
      ...
      {!ready && (
        <div className="loading-state">Loading your learning library…</div>
      )}
      {learningError && (
      ...
      <section className="lms-hero">
        <div>
          <p className="eyebrow">My Studacad</p>
          <h1>Your exam prep dashboard</h1>
          <p>
            Book subject support, watch syllabus-focused tutorials, download
            revision papers, and check your understanding after every lesson.
          </p>
        </div>
      ...
      <section className="lms-section">
        <div className="lms-section-heading">
          ...
          <Link className="outline" href="/wallet">
            Top up wallet
          </Link>
        </div>
        <div className="course-grid">
          {purchased.map((course) => {
            ...
            return (
              <article
                className="course-card owned"
                key={course.id}
                style={
                  { "--course-color": course.color } as React.CSSProperties
                }
              >
                ...
                <div className="course-body">
                  <p className="course-author">With {course.instructor}</p>
                  <h3>{course.title}</h3>
                  <p>{course.description}</p>
                  <div className="progress-row">
                    <span>
                      <b>{done}</b> of {course.lessons.length} lessons
                    </span>
                  ...
                  <div className="lesson-list">
                    {course.lessons.map((lesson, index) => (
                      <Link
                        key={lesson.id}
                        href={`/lesson?course=${course.id}&lesson=${lesson.id}`}
                      >
                        <span
                          className={
                            completedLessonIds.includes(lesson.id)
                        ...
                        <span>
                          ...
                          <small>
                            ...
                            {quizScores[lesson.id] !== undefined
                              ? ` · Best score ${quizScores[lesson.id]}%`
                              : ""}
                          </small>
                        </span>
                        <b>→</b>
      ...
      <section className="lms-section discover-section">
        <div className="lms-section-heading">
          <div>
            <p className="eyebrow">Revision marketplace</p>
            <h2>Add an exam prep course</h2>
          </div>
          <p>
            Purchase once with credits. Every tutorial, paper, and test stays in
            the same Studacad account.
          </p>
        </div>
        <div className="course-grid discover-grid">
            ...
            >
              ...
              <div className="course-body">
                <p className="course-author">
                  {course.lessons.length} lessons · With {course.instructor}
                </p>
                <h3>{course.title}</h3>
                <p>{course.description}</p>
                <div className="course-purchase">
                  <strong>{credit(course.price)}</strong>
                  <button
                    onClick={() => void buy(course.id)}
                    disabled={
                      credits < course.price || purchasing === course.id
                    }
                  >
          ...
          {discover.length === 0 && (
            <div className="empty-state">
              <strong>Your library is full.</strong>
              <p>You own every course currently available.</p>
            </div>
          )}
        </div>
      </section>
