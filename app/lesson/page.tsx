...
export default function LessonPage() {
  ...
  return <main className="lms-page lesson-page">
    ...
    <div className="lesson-shell">
      <aside className="lesson-sidebar"><Link href="/learn">← Learning hub</Link><p className="eyebrow">{course.language} · {course.level}</p><h2>{course.title}</h2><div className="side-lessons">{course.lessons.map((item, index) => <Link key={item.id} className={item.id === lesson.id ? "active" : ""} href={`/lesson?course=${course.id}&lesson=${item.id}`}><span className={completedLessonIds.includes(item.id) ? "complete" : ""}>{completedLessonIds.includes(item.id) ? "✓" : index + 1}</span><span><strong>{item.title}</strong><small>{item.duration}</small></span></Link>)}</div><Link className="wallet-mini" href="/wallet">◆ Earn 10 credits for passing</Link></aside>
      <article className="lesson-content"><div className="lesson-title"><div><p className="eyebrow">Lesson · {lesson.duration}</p><h1>{lesson.title}</h1><p>{lesson.description}</p></div>{quizScores[lesson.id] !== undefined && <span className="best-score">Best score<strong>{quizScores[lesson.id]}%</strong></span>}</div>
        ...
        <section className="revision-card"><span className="paper-icon">▤</span><div><p className="eyebrow">Revision paper</p><h2>{lesson.revisionTitle}</h2><p>Keep the key phrases and exercises for offline revision.</p></div><button className="outline" onClick={downloadPaper}>{/^https?:\/\//i.test(lesson.revisionContent) ? "Open paper" : "Download paper"} ↓</button></section>
