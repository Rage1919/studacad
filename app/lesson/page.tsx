...
export default function LessonPage() {
  ...
  return <main className="lms-page lesson-page">
    ...
    <div className="lesson-shell">
      <aside className="lesson-sidebar"><Link href="/learn">← My learning</Link><p className="eyebrow">{course.examination} · {course.subject} · {course.level}</p><h2>{course.title}</h2><div className="side-lessons">{course.lessons.map((item, index) => <Link key={item.id} className={item.id === lesson.id ? "active" : ""} href={`/lesson?course=${course.id}&lesson=${item.id}`}><span className={completedLessonIds.includes(item.id) ? "complete" : ""}>{completedLessonIds.includes(item.id) ? "✓" : index + 1}</span><span><strong>{item.title}</strong><small>{item.duration}</small></span></Link>)}</div><Link className="wallet-mini" href="/wallet">◆ Earn 10 credits for passing</Link></aside>
