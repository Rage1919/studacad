...
export function TutorApplicationForm() {
  ...
  return (
    <form className="tutor-application-form" action="profile" onSubmit={submit}>
      <div className="tutor-form-row">
        <label>Full name<input autoComplete="name" required /></label>
        <label>Email address<input type="email" autoComplete="email" required /></label>
      </div>
      <div className="tutor-form-row">
        <label>Teaching level<select defaultValue="" required><option value="" disabled>Choose a level</option><option>PSLE</option><option>JCE</option><option>BGCSE</option><option>More than one</option></select></label>
        <label>Strongest subject<input placeholder="e.g. Mathematics" required /></label>
      </div>
