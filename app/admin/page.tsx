...
export default function AdminPage() {
  ...
  const [courseForm, setCourseForm] = useState({ title: "", examination: "PSLE" as ExamLevel, subject: "Mathematics", level: "Standard 7", description: "", instructor: "", price: 120, color: "#c9fa78" });
  ...
  const createCourse = (event: FormEvent) => {
    ...
    setCourseForm({ title: "", examination: "PSLE", subject: "Mathematics", level: "Standard 7", description: "", instructor: "", price: 120, color: "#c9fa78" });
    setTab("lesson");
  ...
  return <main className="lms-page admin-page">
    ...
    {tab === "course" && <section className="admin-workspace"><div className="form-intro"><span>01</span><div><p className="eyebrow">Create a course</p><h2>Set up the exam learning path.</h2><p>Learners purchase the course once with credits and receive every tutorial, revision paper, and test you publish inside it.</p></div></div><form className="admin-form" onSubmit={createCourse}><div className="field-grid"><label>Course title<input required value={courseForm.title} onChange={e => setCourseForm({ ...courseForm, title: e.target.value })} placeholder="e.g. BGCSE Physics Paper Skills" /></label><label>Instructor<input required value={courseForm.instructor} onChange={e => setCourseForm({ ...courseForm, instructor: e.target.value })} placeholder="Tutor name" /></label><label>Examination<select value={courseForm.examination} onChange={e => setCourseForm({ ...courseForm, examination: e.target.value as ExamLevel })}><option>PSLE</option><option>JCE</option><option>BGCSE</option></select></label><label>Subject<input required value={courseForm.subject} onChange={e => setCourseForm({ ...courseForm, subject: e.target.value })} placeholder="e.g. Mathematics" /></label><label>Class level<input required value={courseForm.level} onChange={e => setCourseForm({ ...courseForm, level: e.target.value })} placeholder="e.g. Form 5" /></label><label>Price in credits<input type="number" min="1" required value={courseForm.price} onChange={e => setCourseForm({ ...courseForm, price: Number(e.target.value) })} /></label><label>Course color<input type="color" value={courseForm.color} onChange={e => setCourseForm({ ...courseForm, color: e.target.value })} /></label></div><label>Description<textarea required rows={4} value={courseForm.description} onChange={e => setCourseForm({ ...courseForm, description: e.target.value })} placeholder="Which Botswana examination skills will students master?" /></label><button className="primary" type="submit">Create course →</button></form></section>}
    ...
    {tab === "library" && <section className="admin-workspace content-library"><div className="form-intro"><span>03</span><div><p className="eyebrow">Content library</p><h2>Everything published.</h2><p>Review the current PSLE, JCE, and BGCSE course structure, resources, and assessment counts.</p></div></div><div className="admin-course-list">{courses.map(course => <article key={course.id}><div className="admin-course-head" style={{ borderColor: course.color }}><span>{course.examination}</span><div><h3>{course.title}</h3><p>{course.subject} · {course.level} · {course.price} credits · {course.instructor}</p></div><strong>{course.lessons.length} lessons</strong></div>{course.lessons.map(lesson => <div className="admin-lesson-row" key={lesson.id}><span>▶</span><div><strong>{lesson.title}</strong><small>{lesson.duration} · {lesson.revisionTitle}</small></div><b>{lesson.quiz.length} MCQ</b></div>)}</article>)}</div><button className="reset-demo" onClick={() => { if (window.confirm("Reset all demo content, credits, purchases, and scores?")) { resetDemo(); flash("Demo data reset."); } }}>Reset demo data</button></section>}
  </main>;
}
